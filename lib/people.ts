...
export function workspaceMutate(s:State,u:Row,p:any):any{
 ...
 if(p.op==='company.invite'){
  ...
  const i={id:uid(),email,name,mobile,token:uid()+uid(),expires:new Date(Date.now()+7*86400000).toISOString(),state:'pending',delivery:'queued',inviter:u.id,at:now()};
 ...
 if(p.op==='invite.accept'){
  ...
  i.state='accepted';u.onboarding=false;u.name=i.name||u.name;u.mobile=i.mobile||'';activateAssignments(s,u);audit(s,u,'',i.id,'Company invitation accepted',null,u.email);

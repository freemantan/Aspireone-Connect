...
export function PlanningOnlineBudget({budget:b,rules,year,editable,onChange}:Props){
 ...
 const changeProductSales=(id:string,v:number|null)=>{const n=clone(b),line=n.lines.find(l=>l.id===id)!;if(line.method!=='manual')return;line.values=Array.from({length:12},(_,i)=>v===null?null:Math.floor(v/12)+(i<v%12?1:0));if(n.cashflow)n.cashflow.annualTarget=null;onChange(n);};
 ...
 return <div className="online-budget">
  ...
  <section id="online-volume"><h3>4. External sales value and volume</h3><p>Illustrative equivalents, not unique students: Product C is a 12-class block; Product D is a one-subject year. The initial averages below come from the v4 report. Adjust them for the expected sales mix. Average block/unit prices are temporary conversion inputs; annual dollar sales are saved with the budget.</p><fieldset disabled={!editable}><div className="planning-fields online-sales-values">{(['c','d'] as const).map(id=><label key={id}>Product {id.toUpperCase()} annual external sales (S$)<Num label={'Product '+id.toUpperCase()+' annual external sales'} money value={annual(result.rows[id]||[null])} disabled={b.lines.find(l=>l.id===id)?.method!=='manual'} onChange={(v:number|null)=>changeProductSales(id,v)}/></label>)}<div><small>Total annual external sales</small><strong>{money(external)}</strong></div></div></fieldset><p>Dollar sales above drive the budget and cashflow. Block and unit equivalents below are supporting estimates.</p><div className="planning-fields"><label>Average Product C 12-class block<Num money value={cFee} disabled={!editable} step="1" onChange={(v:number|null)=>setCFee(v??0)}/></label><label>Average Product D one-subject year<Num money value={dFee} disabled={!editable} step="1" onChange={(v:number|null)=>setDFee(v??0)}/></label></div><div className="planning-scroll"><table><thead><tr><th>Product</th><th>Budget equivalents</th><th>Breakeven equivalents</th></tr></thead><tbody>{[['c','C · 12-class blocks',cFee],['d','D · subject-years',dFee]].map(([id,label,fee])=><tr key={id}><th>{label}</th>{[result,targetRows].map((r,i)=>{const v=r?annual(r.rows[id]||[null]):null;return <td key={i}>{v===null||!fee?'Pending':Math.ceil(v/Number(fee)).toLocaleString('en-SG')}</td>;})}</tr>)}</tbody></table></div></section>

...
function PersonEditor({person,kind,run,busy}:any){
 const [editing,setEditing]=useState(false),[name,setName]=useState(person.name||''),[mobile,setMobile]=useState(person.mobile||'');
 if(!editing)return <button disabled={busy} onClick={()=>{setName(person.name||'');setMobile(person.mobile||'');setEditing(true);}}>Edit details</button>;
 return <form className="detail-body" onSubmit={async e=>{e.preventDefault();if(await run({op:'person.edit',kind,id:person.id,name,mobile}))setEditing(false);}}><label>Person Short Name<input required maxLength={150} value={name} onChange={e=>setName(e.target.value)}/></label><label>Mobile number (optional)<input type="tel" maxLength={40} value={mobile} onChange={e=>setMobile(e.target.value)}/></label><button className="primary" disabled={busy}>Save details</button><button type="button" disabled={busy} onClick={()=>setEditing(false)}>Cancel</button></form>;
}
export function AdministrationPeople({s,act}:any){
 ...
 <label>Person Short Name<input required maxLength={150} value={name} onChange={e=>setName(e.target.value)}/></label>
 ...
 <h3>Invitations</h3>{!s.invites.length&&<p>No invitations yet.</p>}{s.invites.map((i:any)=><div className="member-row" key={i.id}><span>{i.name||i.email}<small>{i.email}{i.mobile?' · '+i.mobile:''}</small><small>{i.state==='pending'&&i.expires<new Date().toISOString()?'Expired':i.state} · {i.delivery}{i.board?' · Earlier board invitation':''}</small></span><PersonEditor person={i} kind="invites" run={run} busy={busy}/><button disabled={busy} onClick={()=>{if(confirm(i.state==='accepted'?'Remove this invitation record? The accepted user keeps their board access.':'Remove this invitation and invalidate its link? Pending board assignments are removed if no other invitation remains.'))run({op:'invite.remove',id:i.id});}}>Remove invitation</button>{i.state!=='accepted'&&<><button disabled={busy} onClick={()=>run({op:'invite.update',id:i.id})}>Resend</button>{i.state==='pending'&&<button disabled={busy} onClick={()=>{if(confirm('Cancel this invitation and remove pending board assignments?'))run({op:'invite.update',id:i.id,cancel:true});}}>Cancel invitation</button>}</>}</div>)}
 <h3>System users</h3><p>Board Manage access never grants system administration.</p>{s.users.filter((u:any)=>!u.deleted).map((u:any)=><div className="member-row" key={u.id}><span>{u.name}<small>{u.email}{u.mobile?' · '+u.mobile:''}</small></span><PersonEditor person={u} kind="users" run={run} busy={busy}/><button disabled={busy||u.id===s.me.id} onClick={()=>{if(confirm('Delete '+u.name+' from the portal? Access and memberships will be removed. Past tasks and messages will be retained.'))run({op:'user.delete',id:u.id});}}>Delete user</button><label><input type="checkbox" checked={!!u.active} disabled={busy} onChange={e=>run({op:'user',id:u.id,active:e.target.checked})}/> Active</label><label><input type="checkbox" checked={!!u.admin} disabled={busy} onChange={e=>{if(confirm('Change global administrator access for '+u.email+'?'))run({op:'user',id:u.id,admin:e.target.checked});}}/> Administrator</label></div>)}</div>;
}

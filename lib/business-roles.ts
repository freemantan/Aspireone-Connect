...
export const businessRoles=['Shareholder','Director','Franchisee','Business Manager','Subject Head','Customer Relationship','Finance','HR','Manager'];
...
export function roleAccess(s:State,u:Row,board:string){
 ...
 return Math.max(0,...(u.roles||[]).map((r:string)=>accessLevel(grants[r])));
}

...
export function PlanningOnlineBudget({budget:b,rules,year,editable,onChange}:Props){
 ...
 return <div className="online-budget">
  ...
  <nav className="online-nav" aria-label="Budget report sections">{[['assumptions','Assumptions'],['overview','At a glance'],['ah','AH revenue'],['costs','Cost budget'],['breakeven','Breakeven'],['volume','Sales volumes'],['sensitivity','Sensitivity'],['monthly','Monthly projection']].map(([id,label])=><a key={id} href={'#online-'+id}>{label}</a>)}</nav>
  <section id="online-assumptions"><h3>Assumptions — edit the budget</h3><p>Amounts are in Singapore dollars. Blank values remain pending. The detailed P&L editor below retains additional lines and individual monthly adjustments.</p>
   <fieldset disabled={!editable}><div className="planning-fields">
    ...
    {b.lines.filter(l=>['teachers','cc','dc'].includes(l.id)&&l.method==='percent'&&(l.rateSource||'custom')==='custom').map(l=><label key={l.id}>{l.name} (%)<Num value={l.rate} max={100} step="0.1" onChange={(v:number|null)=>changeLine(l.id,v,true)}/><small>{l.id==='teachers'?'Share of all Product C revenue.':`Effective rate on all Product ${l.id==='cc'?'C':'D'} revenue; v4 uses 30% referred × ${l.id==='cc'?'15% = 4.5%':'40% = 12%'}.`}</small></label>)}
    {b.lines.filter(l=>l.id==='ah'&&manual(l)).map(l=><label key={l.id}>AH access fees per month<Num money value={l.values[0]} step="1" onChange={(v:number|null)=>changeLine(l.id,v)}/><small>v4 base: 2,833 eligible account-terms × S$12 × 3 = S$101,988/year.</small></label>)}
   </div></fieldset>

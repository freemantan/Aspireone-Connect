...
test('online cashflow covers Sep 2026 to Dec 2027, reconciles launch payments and opening balances',()=>{
 ...
 for(const l of b.lines.filter(l=>l.category==='revenue')){
  ...
  if(l.id==='ah')assert.deepEqual(row.values.slice(0,4),[0,0,0,0]);else assert.deepEqual(row.values.slice(0,4),seasonalCollections(annual(base.rows[l.id])).slice(8));
 }
...
test('cashflow breakeven covers the full period without consuming opening cash',()=>{
 ...
 assert.deepEqual(flow.rows.find(l=>l.id==='ah')!.values,[...Array(4).fill(0),...Array(12).fill(849900)]);
 assert.equal(JSON.stringify(b),before);
 b.lines.find(l=>l.id==='tech')!.values=Array(12).fill(900000);
 assert.ok(cashflowBreakeven(b,seedRules,.6)!.external>target.external);
});

import {linkedOnlineCommissions} from '../lib/online-commissions';
test('online referrals follow published commission totals while keeping referral share and saved inputs',()=>{
 const b=budgetTemplate(true),r=clone(seedRules);b.lines.find(l=>l.id==='cc')!.driver={kind:'referral',referred:40,commission:99};
 r.rates.initial['Physical Centre'].C=[5,5,10,0];
 const linked=linkedOnlineCommissions(b,r);assert.equal(linked.lines.find(l=>l.id==='cc')!.rate,8);
 assert.equal(b.lines.find(l=>l.id==='cc')!.driver!.kind,'referral');

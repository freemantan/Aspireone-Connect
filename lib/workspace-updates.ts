...
export function updateCommonColumns(s:State){
 const board=s.boards.find(b=>b.id==='board-1');if(!board||board.commonColumnsUpdated)return false;
 const removed=board.columns.filter((c:any)=>/^custom links?$/i.test(c.name.trim()));
 ...
 board.columns=board.columns.filter((c:any)=>!removed.includes(c));board.hidden=(board.hidden||[]).filter((id:string)=>!removed.some((c:any)=>c.id===id));board.commonColumnsUpdated=true;board.version++;return true;
}

...
import {breakeven,onlineSales,salesProfile,withOnlineSales} from '@/lib/online-budget';
...
export function PlanningOnlineBudget({budget:b,rules,year,editable,onChange}:Props){
 ...
 return <div className="online-budget">
  ...
  <section id="online-breakeven"><h3>3. Annual operating breakeven reference</h3><p>This reference preserves the annual budget’s other income, costs, taxes and saved price/commission assumptions. Only external C and D sales change, holding the annual product mix at {pct(share)} : {pct(1-share)}.</p>
   {!target?<p className="warning">A breakeven target cannot be calculated while values are pending or external sales do not produce a positive contribution. Review the detailed budget inputs.</p>:<>
    <p>Full-year external sales required: <strong>{money(target.external)}</strong>. This is an annual operating-cost reference; the cashflow statement below includes the specified launch-payment schedule and opening cash.</p>
    {editable&&guided&&<button onClick={()=>changeSales(target.external,share,1)}>Use annual breakeven collection target</button>}
   </>}
  </section>
